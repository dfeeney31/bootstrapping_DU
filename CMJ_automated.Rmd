...
confidenceDifferences_m <- function(df, metric, num_boots) {
  C1_ap <- subset(df[,metric], df$Condition == 'M')
  LC_ap <- subset(df[,metric], df$Condition == 'S')
...
confidenceDifferences_l <- function(df, metric, num_boots) {
  C1_ap <- subset(df[,metric], df$Condition == 'L')
  LC_ap <- subset(df[,metric], df$Condition == 'S')
...
confidenceDifferences_t <- function(df, metric, num_boots) {
  C1_ap <- subset(df[,metric], df$Condition == 'T')
  LC_ap <- subset(df[,metric], df$Condition == 'S')
...
write.csv(results, file = 'DU_20subject_summary_CMJ.csv')
```
...
p + theme_classic() + ylab('Total Categorical Score') + xlab('Configuration') + ggtitle('Counter Movement Jump')
