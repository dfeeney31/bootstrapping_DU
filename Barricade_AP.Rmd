...
confidenceDifferences_BS <- function(df, metric, num_boots) {
  C1_ap <- subset(df[,metric], df$Condition == 'B' & df$Movement == 'AP') #Pick a Boa config
  LC_ap <- subset(df[,metric], df$Condition == 'S' & df$Movement == 'AP') #This is the laced config
...
confidenceDifferences_BL <- function(df, metric, num_boots) {
  C1_ap <- subset(df[,metric], df$Condition == 'L' & df$Movement == 'AP') #Boa shroud
  LC_ap <- subset(df[,metric], df$Condition == 'S' & df$Movement == 'AP') #This is shroud laceless
...
confidenceDifferences_t <- function(df, metric, num_boots) {
  C1_ap <- subset(df[,metric], df$Condition == 'R' & df$Movement == 'AP') #Shroud laceless
  LC_ap <- subset(df[,metric], df$Condition == 'S' & df$Movement == 'AP') #Laced
...
```{r}
skate <- subset(s5, s5$Movement == 'AP')
depVar <- 'TimeonFP'
...
results <- as.data.frame(cbind(as.vector(cbind(rep('BOA_to_Lace',length(lCategorical_score)), rep('Boa_to_LL',length(mCategorical_score)), rep('DD_to_SD',length(tCategorical_score)))), as.vector(cbind(lCategorical_score, mCategorical_score, tCategorical_score)), as.vector(rbind(rep(unique(skate$Subject),3)))))
...
write.csv(results, file = 'Performance_testAP_5.csv')
```
